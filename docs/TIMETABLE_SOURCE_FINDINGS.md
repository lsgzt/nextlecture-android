...
Final behavior: resolve the current sub-section timetable link from the permanent index on every foreground refresh. If the official index is unavailable or cannot be parsed, use the configured `Timetable_url` server value, then the user’s saved URL, then the built-in current URL. The resolver avoids relying on a dated URL during normal operation, restricts matches to anchor text containing “Sub-section wise” / “Subsection wise”, and allows only HTTPS appsc.gndec.ac.in URLs to prevent arbitrary redirect sources.
